...
exports.HttpServer = exports.Heart = exports.HttpProvider = exports.AuthType = void 0;
...
var HttpProvider = /** @class */ (function () {
     ...
     */
    HttpProvider.prototype.replaceTemplates = function (route, response, extraOptions) {
        var base = this.base(route);
        var options = __assign({ base: base, csStaticBase: base + "/static/" + this.options.commit + this.rootPath, logLevel: logger_1.logger.level }, extraOptions);
        response.content = response.content
    ...
    HttpProvider.prototype.ensureMethod = function (request, method) {
        var check = util_1.arrayify(method || "GET");
...
var HttpServer = /** @class */ (function () {
            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    ...
                    case 4:
                        ...
                        logger_1.logger.debug("Request error", logger_1.field("url", request.url), logger_1.field("code", code), logger_1.field("error", error_1));
                        ...
                        write({
                            ...
                            content: __assign({ error: e.message }, (e.details || {})),
                        });
                        return [3 /*break*/, 7];
                    case 5:
                        _b = write;
        ...
        this.heart = new Heart(path.join(util_2.paths.data, "heartbeat"), function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        connections = _a.sent();
                        logger_1.logger.trace(util_1.plural(connections, connections + " active connection"));
    ...
    HttpServer.prototype.dispose = function () {
        return __awaiter(this, void 0, void 0, function () {
            var providers, responses;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        this.socketProvider.stop();
                        providers = Array.from(this.providers.values());
                        return [4 /*yield*/, Promise.all(providers.map(function (p) { return p.dispose().catch(function (e) { return e; }); }))];
                    case 1:
                        responses = _a.sent();
                        return [2 /*return*/, responses.filter(function (r) { return typeof r !== "undefined"; })];
                }
            });
        });
    };
    ...
    HttpServer.prototype.registerHttpProvider = function (endpoint, provider) {
        ...
        var endpoints = util_1.arrayify(endpoint).map(util_1.trimSlashes);
        endpoints.forEach(function (endpoint) {
            if (/\//.test(endpoint)) {
                throw new Error("Only top-level endpoints are supported (got " + endpoint + ")");
            }
            var existingProvider = _this.providers.get("/" + endpoint);
            _this.providers.set("/" + endpoint, p);
            if (existingProvider) {
                logger_1.logger.debug("Overridding existing /" + endpoint + " provider");
                // If the existing provider isn't registered elsewhere we can dispose.
                if (!Array.from(_this.providers.values()).find(function (p) { return p === existingProvider; })) {
                    logger_1.logger.debug("Disposing existing /" + endpoint + " provider");
    ...
    HttpServer.prototype.parseUrl = function (request) {
        ...
        if (base !== "/" && provider) {
            return __assign(__assign({}, parse(requestPath)), { providerBase: base, fullPath: fullPath, query: parsedUrl.query, provider: provider, originalPath: originalPath });
        }
        ...
        return { base: base, providerBase: "/", fullPath: fullPath, requestPath: requestPath, query: parsedUrl.query, provider: provider, originalPath: originalPath };
    };
    /**
    ...
    HttpServer.prototype.doProxy = function (route, request, response, options) {
        ...
        // the event.
        ;
        request.base = options.strip;
        var isHttp = response instanceof http.ServerResponse;
        var base = options.strip ? route.fullPath.replace(options.strip, "") : route.fullPath;
        var path = util_1.normalize("/" + (options.prepend || "") + "/" + base, true);
        var proxyOptions = {
            changeOrigin: true,
            ignorePath: true,
